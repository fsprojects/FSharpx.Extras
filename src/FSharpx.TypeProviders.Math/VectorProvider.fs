          ...
          (fun typeName parameterValues ->
                ...
                vectorType
                    |+!> (provideConstructor
                           ...
                           |> addConstructorXmlDoc "Initializes a vector instance")
                    |+!> (provideMethod
                            "DotProduct"
                            ["factor", vectorType]
                            typeof<float>
                            (fun [this; other] -> <@@ dotProduct %%this %%other @@>)
                           |> addMethodXmlDoc "Calculates the dot product with the given factor.")
                    |+!> (provideMethod
                            "Scale"
                            ["factor", typeof<float>]
                            vectorType
                            (fun [this; factor] -> <@@ scale %%this %%factor @@>)
                           |> addMethodXmlDoc "Calculates the scalar multiplication with the given factor.")
                    |+!> (provideMethod
                            "Add"
                            ["summand", vectorType]
                            vectorType
                            (fun [this; other] -> <@@ add %%this %%other @@>)
                           |> addMethodXmlDoc "Calculates the sum with the given summand.")
                    |+!> (provideMethod
                            "Subtract"
                            ["subtrahend", vectorType]
                            vectorType
                            (fun [this; other] -> <@@ subtract %%this %%other @@>)
                           |> addMethodXmlDoc "Calculates the difference with the given subtrahend.")
                    |+!> (provideMethod
                            "Equals"
                            ["other", vectorType]
                            typeof<bool>
                            (fun [this; other] -> <@@ (%%this:float[]) = %%other @@>)
                           |> addMethodXmlDoc "Returns wether the given objects are equal.")
                    |++!> (parameters
                            |> Seq.mapi (fun i name ->
                                    provideProperty 
                                        name
