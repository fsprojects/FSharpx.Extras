    ...
    let extractEnum className d (wmiProp:PropertyData) =
        ...
        try
            for valName, valValue in namesAndVals do 
                ...
                let isProp = ProvidedProperty("Is_"+valName.Replace(" ","_"),typeof<bool>,GetterCode=(fun args -> <@@ (%% valValueRepr : obj).Equals(%%(Expr.Coerce(args.[0], typeof<obj>))) @@>))
                ...
                if wmiMeth.Origin = wmiClass.ClassPath.ClassName then
                    ...
                    let p = ProvidedMethod(methName,parameters @ outParameters |> List.map fst,retTy, 
                                            InvokeCode = (fun args -> 
                                                ...
                                                let call = 
                                                    if isStatic then
                                                        Expr.Call(runtimeInfo.InvokeStaticManagementMethod, [ Expr.Value wmiClass.ClassPath.Path; Expr.Value methName; inParamNamesExpr; inParamValuesExpr])                                                         
                                                    else
