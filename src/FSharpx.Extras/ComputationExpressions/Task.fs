...
module Task =
    ...
    let internal run (t: unit -> Task<'a>) : Result<'a> = 
        try
            let task = t()
    ...
    type internal TaskBuilder(?continuationOptions, ?scheduler, ?cancellationToken) =
    ...
    let internal task = TaskBuilder()

    type TokenToTask<'a> = CancellationToken -> Task<'a>
    type internal TaskBuilderWithToken(?continuationOptions, ?scheduler) =
        let contOptions = defaultArg continuationOptions TaskContinuationOptions.None
        let scheduler = defaultArg scheduler TaskScheduler.Default
    ...
    let WhenAllUnits (units:seq<Task<unit>>) : Task<unit> =
        task {
