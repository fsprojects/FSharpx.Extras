...
module Enumerator = 
    ...
    let private skipWithPredicate pred (en:IEnumerator<'a>) =
        let rec loop () =
            if not (en.MoveNext()) then empty<_>
            elif pred en.Current then
                en.MoveNext() |> ignore
                en
            else loop ()
        loop ()

    let skipUntil pred en = skipWithPredicate (not << pred) en
  
    let skipWhile pred en = skipWithPredicate pred en
    ...
    let private takeWithPredicate pred en =
        let rec loop () = iter {
            let! v = en
            match v with
            | None -> ()
    ...
    let takeUntil pred en = takeWithPredicate (not << pred) en

    let takeWhile pred en = takeWithPredicate pred en
    ...
    /// Applies a transformation on all items generated by the enumerator.
    ...
    let private scanWithPredicate f state pred en = iter {
        yield state
        let! x = en
        match x with
        | None -> ()
    ...
    /// Folds over the enumerator immediately, returning the final state of the accumulator.
    let fold f state en = scan f state en |> last
