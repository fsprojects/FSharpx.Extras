// ----------------------------------------------------------------------------
// F# TrieMap_SU implementation (TrieMap_SU.fs)
...
// Current TrieMap_SU implementation is as a struct of 2 elements, trying limiting allocation on the outer node creation.
...
type TrieMap_SU<'Key, 'T when 'Key : equality> =
    struct
    ...
    static member inline private settleCollision(originalNode : HKVNode<'Key, 'T>) (newNode : HKVNode<'Key, 'T>) bucketShift : Flagged_SU<Object> =
        if newNode.Key = originalNode.Key then // this takes care of the singleton on singleton AND the head of the list
            ...
            new Flagged_SU<Object>(newNode :> Object, false)
        else
            let rec run originalNode newNode bucketShift = // bit of a speed boost here, can inline outer function, and not repeat the first (check for equal) clause.
                if bucketShift = TrieMapConstants.shiftAtWhichYouHitBottom then
                    ...
                    | false->
                        ...
                        new Flagged_SU<Object>(newNode :> Object, true)
                    | true -> // found ourselves, have to overwrite.  Trust afterFound now.
                        ...
                        new Flagged_SU<Object>(rl :> Object, false) // no net add
                else
                    ...
                    if originalsIndex = newItemIndex then
                        ...
                        new Flagged_SU<Object>(arr :> Object, result.Flag)
                    else
                        ...
                        new Flagged_SU<Object>(arr :> Object, true)
            run originalNode newNode bucketShift

    // flag returns True if there was a net new node added, false if (overwrite) keeps count constant
    static member inline private augmentedArray(originalArray : Object array) (newNode : HKVNode<'Key, 'T>) bucketShift : Flagged_SU<Object array> =
        let rec run (originalArray : Object array) newNode bucketShift = // this inner recursive, plus the inline on the outer that it allowed, gave a speed boost
            ...
            match existing with
            | null -> cloned.[index] <- newNode :> Object; new Flagged_SU<Object array>(cloned, true)
            | :? (Object array) as subArray ->
                ...
                new Flagged_SU<Object array>(cloned, result.Flag)
            | :? HKVNode<'Key, 'T> as node ->
                let result = TrieMap_SU<'Key, 'T>.settleCollision node newNode (bucketShift + TrieMapConstants.bucketCountShift)
                cloned.[index] <- result.Value
                new Flagged_SU<Object array>(cloned, result.Flag)
            | _ -> failwith "unknown type"
        run originalArray newNode bucketShift

    member inline private this.withAddition(k : 'Key, v : 'T) : TrieMap_SU<'Key, 'T> =
        ...
        | :? (Object array) as arr ->
            let result = TrieMap_SU.augmentedArray arr newNode 0
            TrieMap_SU((if result.Flag then this.count + 1 else this.count), result.Value)
        | null -> // first node
            new TrieMap_SU<'Key, 'T>(1, (newNode :> Object))
        | :? HKVNode<'Key, 'T> as node ->
            let result = TrieMap_SU.settleCollision node newNode 0
            TrieMap_SU((if result.Flag then this.count + 1 else this.count), result.Value)
        | _ -> failwith "unknown type"
    ...
    static member inline private deleteFromNodeList nodeList (k : 'Key) : Flagged_SU<Object> =
        let mutable found = false
        ...
        if found then
            ...
            while (not (current.Key = k)) do
                rl <- { current with Next = rl }
                current <- current.Next
            new Flagged_SU<Object>(rl :> Object, true)
        else
            new Flagged_SU<Object>(nodeList, false)
    ...
    static member inline private getArrayDeleteResultWithIndexRemoved(originalArray : Object array) (index : int) =
        let originalArrayElementCount = TrieMap_SU<'Key, 'T>.getOriginalArrayElementCount originalArray
        if (originalArrayElementCount = 1) then
            new Flagged_SU<Object>(null, true) // deletion produced nothing, and it was the only thing in the array.
        else
            if originalArrayElementCount = 2 then // for the case when there's only one other thing; but it's a Node not an array
                ...
                | :? (HKVNode<'Key, 'T>) as node when System.Object.ReferenceEquals(node.Next, null) ->
                        new Flagged_SU<Object>(node, true) // return the node instead of the array, proliferate a loner back up the chain
                | _ -> // must be a sub-array, or multiple nodes at bottom level, either being there for a reason for a reason - keep this an array.
                    ...
                    new Flagged_SU<Object>(arr, true)
    ...
    static member private deletedFromArray(originalArray : Object array) (k : 'Key) (h : int) bucketShift : Flagged_SU<Object>  =
        let index = (h >>> bucketShift) &&& TrieMapConstants.bucketMask
        let existing = originalArray.[index]

        match existing with
        | null -> new Flagged_SU<Object>(originalArray, false)
        | :? (Object array) as subArray ->
            let result = TrieMap_SU<'Key, 'T>.deletedFromArray subArray k h (bucketShift + TrieMapConstants.bucketCountShift)
            if result.Flag then
                ...
                | null -> // see if there's only one
                    TrieMap_SU<'Key, 'T>.getArrayDeleteResultWithIndexRemoved originalArray index
                ...
                | :? (HKVNode<'Key, 'T>) as node -> // array delete returned a kvnode - only reason could be is if a lower list went to length of 1 and we're floating it up.
                    if (TrieMap_SU<'Key, 'T>.getOriginalArrayElementCount originalArray) = 1 then
                        new Flagged_SU<Object>(node, true)
                    else
                        let arr = Array.copy originalArray
                ...
                new Flagged_SU<Object>(originalArray, false)
        | :? HKVNode<'Key, 'T> as node ->
            let result = TrieMap_SU<'Key, 'T>.deleteFromNodeList node k
            if result.Flag then // something was deleted
                match result.Value with
                | null -> TrieMap_SU<'Key, 'T>.getArrayDeleteResultWithIndexRemoved originalArray index
                | :? (HKVNode<'Key, 'T>) as node ->
                    if ((TrieMap_SU<'Key, 'T>.getOriginalArrayElementCount originalArray) = 1) && (System.Object.ReferenceEquals(node.Next, null)) then // solo solo - return it
                        new Flagged_SU<Object>(node, true)
                    else
                        let arr = Array.copy originalArray
                        arr.[index] <- node :> Object
    ...
    member inline private this.withRemoval(key : 'Key)  =
        match this.rootNode with
        | null -> TrieMap_SU(0, Unchecked.defaultof<HKVNode<'Key, 'T>>) // already empty
        | :? (Object array) as arr ->
            let result = TrieMap_SU<'Key, 'T>.deletedFromArray arr key (TrieMapConstants.getHashCode key) 0
            if result.Flag then TrieMap_SU(this.count - 1, result.Value) else this
        | :? HKVNode<'Key, 'T> as node ->
            assert System.Object.ReferenceEquals(node.Next, null)
            if node.Key = key then TrieMap_SU(0, null) else this
    ...
    member public this.Add((key : 'Key, value : 'T) as keyValue) : TrieMap_SU<'Key, 'T> = this.withAddition keyValue
    ...
    static member Empty = new TrieMap_SU<'Key, 'T>()
...
module TrieMap_SU =
    ...
    let add key value (table : TrieMap_SU<'Key, 'T>) = table.Add(key, value)
    let remove key (table : TrieMap_SU<'Key, 'T>) = table.Remove key
    let containsKey key (table : TrieMap_SU<'Key, 'T>) = table.ContainsKey
    let empty<'Key, 'T when 'Key : equality> : TrieMap_SU<'Key, 'T> = TrieMap_SU<'Key, 'T>.Empty //new TrieMap_SU<'Key, 'T>()
    let find<'Key, 'T when 'Key : equality> key (table : TrieMap_SU<'Key, 'T>) = table.[key]
    let tryFind<'Key, 'T when 'Key : equality> key (table : TrieMap_SU<'Key, 'T>) = table.TryFind key
    ...
    let ofSeq<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) seq) = elements |> Seq.fold (fun (table : TrieMap_SU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>
    let ofList<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) list) = elements |> List.fold (fun (table : TrieMap_SU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>
    let ofArray<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) array) = elements |> Array.fold (fun (table : TrieMap_SU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>

    let toSeq<'Key, 'T when 'Key : equality> (table : TrieMap_SU<'Key, 'T>) : ('Key * 'T) seq = table :> IEnumerable<'Key * 'T>
    let toList<'Key, 'T when 'Key : equality> (table : TrieMap_SU<'Key, 'T>) : ('Key * 'T) list = table |> Seq.toList
    let toArray<'Key, 'T when 'Key : equality> (table : TrieMap_SU<'Key, 'T>) : ('Key * 'T) array = table |> Seq.toArray
