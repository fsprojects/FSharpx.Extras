// ----------------------------------------------------------------------------
// F# TrieMap_PU implementation (TrieMap_PU.fs)
...
// Current TrieMap_PU implementation is as a struct of 2 elements, trying limiting allocation on the outer node creation.
...
type TrieMap_PU<'Key, 'T when 'Key : equality> =
    struct
    ...
    static member inline private settleCollision(originalNode : HKVNode_PU<'Key, 'T>) (newNode : HKVNode_PU<'Key, 'T>) bucketShift : Flagged_PU<Object> =
        if newNode.Key = originalNode.Key then // this takes care of the singleton on singleton AND the head of the list
            ...
            new Flagged_PU<Object>(newNode :> Object, false)
        else
            let rec run originalNode newNode bucketShift = // bit of a speed boost here, can inline outer function, and not repeat the first (check for equal) clause.
                if bucketShift = TMUtil.shiftAtWhichYouHitBottom then
                    ...
                    let mutable (afterFound : HKVNode_PU<'Key, 'T>) = originalNode // whatever - can't nicely assign it to null, we depend on the flag WasFound
                    ...
                    | false->
                        ...
                        new Flagged_PU<Object>(newNode :> Object, true)
                    | true -> // found ourselves, have to overwrite.  Trust afterFound now.
                        ...
                        new Flagged_PU<Object>(rl :> Object, false) // no net add
                else
                    ...
                    if originalsLogicalIndex = newItemLogicalIndex then
                        ...
                        new Flagged_PU<Object>(TMUtil.singleElementArrayCreateFromLogicalIndex originalsLogicalIndex result.Value, result.Flag)
                    else
                        // they can coexist in the new array
                        new Flagged_PU<Object>(TMUtil.dualElementArrayCreate originalsLogicalIndex originalNode newItemLogicalIndex newNode, true)
            run originalNode newNode bucketShift

    // flag returns True if there was a net new node added, false if (overwrite) keeps count constant
    static member inline private augmentedArray(originalArray : Object array) (newNode : HKVNode_PU<'Key, 'T>) bucketShift : Flagged_PU<Object array> =
        let rec run (originalArray : Object array) newNode bucketShift = // this inner recursive, plus the inline on the outer that it allowed, gave a speed boost
            ...
            | null -> new Flagged_PU<Object array>(TMUtil.arrayWithLogicalInsertion arrayTargetData originalArray logicalIndex newNode, true)
            | elt ->
                ...
                | :? (Object array) as subArray ->
                    ...
                    new Flagged_PU<Object array>(replacementArrayInfo.NewArray, result.Flag)
                | :? HKVNode_PU<'Key, 'T> as node ->
                    let result = TrieMap_PU<'Key, 'T>.settleCollision node newNode (bucketShift + TMUtil.bucketCountShift)
    ...
    member inline private this.withAddition(k : 'Key, v : 'T) : TrieMap_PU<'Key, 'T> =
        let h = TMUtil.getHashCode k
        //let hkv = new HKV<'Key, 'T>(k, v, h)
        let newNode = { Key = k; Value = v; Hash = h; Next = Unchecked.defaultof<HKVNode_PU<'Key, 'T>> }
        match this.rootNode with
        | :? (Object array) as arr ->
            let result = TrieMap_PU.augmentedArray arr newNode 0
            TrieMap_PU((if result.Flag then this.count + 1 else this.count), result.Value)
        | null -> // first node
            new TrieMap_PU<'Key, 'T>(1, (newNode :> Object))
        | :? HKVNode_PU<'Key, 'T> as node ->
            let result = TrieMap_PU.settleCollision node newNode 0
            TrieMap_PU((if result.Flag then this.count + 1 else this.count), result.Value)
        | _ -> failwith "unknown type"
    ...
    static member inline private deleteFromNodeList nodeList (k : 'Key) : Flagged_PU<Object> =
        let mutable found = false
        let mutable (afterFound : HKVNode_PU<'Key, 'T>) = nodeList // whatever - can't nicely assign it to null, we depend on the flag WasFound
        ...
        if found then
            ...
            while (not (current.Key = k)) do
                rl <- { current with Next = rl }
                current <- current.Next
            new Flagged_PU<Object>(rl :> Object, true)
        else
            new Flagged_PU<Object>(nodeList, false)
    ...
    static member inline private getArrayDeleteResultWithIndexRemoved(originalArray : Object array) (logicalIndex : int) =
        ...
        if (originalArrayElementCount = 1) then
            new Flagged_PU<Object>(null, true) // deletion produced nothing, and it was the only thing in the array.
        else
            let arrayTargetData = TMUtil.getArrayTargetData originalArray logicalIndex
            if originalArrayElementCount = 2 then // for the case when there's only one other thing; but it's a Node not an array
                ...
                | :? (HKVNode_PU<'Key, 'T>) as node when System.Object.ReferenceEquals(node.Next, null) ->
                        new Flagged_PU<Object>(node, true) // return the node instead of the array, proliferate a loner back up the chain
                | _ -> // must be a sub-array, or multiple nodes at bottom level, either being there for a reason for a reason - keep this an array.
                    new Flagged_PU<Object>(TMUtil.singleElementArrayCreateFromSingleBitMask bitMaskOfOther otherInArray, true)
            else // > 1 other item in array, so we copy the array up MINUS index = index
                new Flagged_PU<Object>(TMUtil.arrayWithLogicalRemoval arrayTargetData originalArray logicalIndex, true)

    // returns true only if something was actually deleted
    static member private deletedFromArray(originalArray : Object array) (k : 'Key) (h : int) bucketShift : Flagged_PU<Object>  =
        let logicalIndex = (h >>> bucketShift) &&& TMUtil.bucketMask
        let arrayTargetData = TMUtil.getArrayTargetData originalArray logicalIndex

        match arrayTargetData.SomethingAlreadyPresent with
        | null -> new Flagged_PU<Object>(originalArray, false)
        | :? (Object array) as subArray ->
            let result = TrieMap_PU<'Key, 'T>.deletedFromArray subArray k h (bucketShift + TMUtil.bucketCountShift)
            if result.Flag then
                ...
                | null -> // see if there's only one
                    ...
                    | n ->
                        ...
                        if n = 2 then
                            ...
                            | :? (HKVNode_PU<'Key, 'T>) as node
                                when (match (node.Next :> Object) with | null -> true | _ -> false) -> new Flagged_PU<Object>(node, true)
                            | _ -> new Flagged_PU<Object>(newArray, true)
                        else
                            new Flagged_PU<Object>(newArray, true)
                | :? (Object array) as newSubArray -> // pretty simple, sub this in, return as a delete
                    ...
                    new Flagged_PU<Object>(replacementArrayInfo.NewArray, true)
                | :? (HKVNode_PU<'Key, 'T>) as node -> // array delete returned a kvnode - only reason could be is if a lower list went to length of 1 and we're floating it up.
                    if (TMUtil.getOriginalArrayElementCount originalArray) = 1 then
                        new Flagged_PU<Object>(node, true)
                    else
                        let replacementArrayInfo = TMUtil.getReplacementInfo arrayTargetData originalArray
                ...
                new Flagged_PU<Object>(originalArray, false)
        | :? HKVNode_PU<'Key, 'T> as node ->
            let result = TrieMap_PU<'Key, 'T>.deleteFromNodeList node k
            if result.Flag then // something was deleted
                match result.Value with
                | null -> TrieMap_PU<'Key, 'T>.getArrayDeleteResultWithIndexRemoved originalArray logicalIndex
                | :? (HKVNode_PU<'Key, 'T>) as node ->
                ...
                new Flagged_PU<Object>(originalArray, false) // nothing deleted
        | _ -> failwith "Unknown type on delete from array"


    member inline private this.withRemoval(key : 'Key)  =
        match this.rootNode with
        | null -> TrieMap_PU(0, Unchecked.defaultof<HKVNode_PU<'Key, 'T>>) // already empty
        | :? (Object array) as arr ->
            let result = TrieMap_PU<'Key, 'T>.deletedFromArray arr key (TMUtil.getHashCode key) 0
            if result.Flag then TrieMap_PU(this.Count - 1, result.Value) else this
        | :? HKVNode_PU<'Key, 'T> as node ->
            assert System.Object.ReferenceEquals(node.Next, null)
            if node.Key = key then TrieMap_PU(0, null) else this
    ...
    member public this.Add((key : 'Key, value : 'T) as keyValue) : TrieMap_PU<'Key, 'T> = this.withAddition keyValue
    ...
    static member Empty = new TrieMap_PU<'Key, 'T>()
...
and
 private HKVNode_PU<'Key, 'T when 'Key : equality> = {
    Key : 'Key
    Value : 'T
    Hash : int
    mutable Next : HKVNode_PU<'Key, 'T> // this lets HKVNode_PU perform double-duty, being mutated when put in place after being passed down a chain.  Never mutated once placed
    }
...
module TrieMap_PU =
    ...
    let add key value (table : TrieMap_PU<'Key, 'T>) = table.Add(key, value)
    let remove key (table : TrieMap_PU<'Key, 'T>) = table.Remove key
    let containsKey key (table : TrieMap_PU<'Key, 'T>) = table.ContainsKey
    let empty<'Key, 'T when 'Key : equality> : TrieMap_PU<'Key, 'T> = TrieMap_PU<'Key, 'T>.Empty //new TrieMap_PU<'Key, 'T>()
    let find<'Key, 'T when 'Key : equality> key (table : TrieMap_PU<'Key, 'T>) = table.[key]
    let tryFind<'Key, 'T when 'Key : equality> key (table : TrieMap_PU<'Key, 'T>) = table.TryFind key
    ...
    let ofSeq<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) seq) = elements |> Seq.fold (fun (table : TrieMap_PU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>
    let ofList<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) list) = elements |> List.fold (fun (table : TrieMap_PU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>
    let ofArray<'Key, 'T when 'Key : equality> (elements : ('Key * 'T) array) = elements |> Array.fold (fun (table : TrieMap_PU<'Key, 'T>) newKV -> table.Add newKV ) empty<'Key, 'T>

    let toSeq<'Key, 'T when 'Key : equality> (table : TrieMap_PU<'Key, 'T>) : ('Key * 'T) seq = table :> IEnumerable<'Key * 'T>
    let toList<'Key, 'T when 'Key : equality> (table : TrieMap_PU<'Key, 'T>) : ('Key * 'T) list = table |> Seq.toList
    let toArray<'Key, 'T when 'Key : equality> (table : TrieMap_PU<'Key, 'T>) : ('Key * 'T) array = table |> Seq.toArray
