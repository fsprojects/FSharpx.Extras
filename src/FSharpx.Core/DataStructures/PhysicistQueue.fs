...
type PhysicistQueue<'a> (prefix : list<'a>, frontLength : int, front : Lazy<list<'a>>, rBackLength : int, rBack : list<'a>) = 
    ...
    static member private check (q : PhysicistQueue<'a>) =
        if q.rBackLength <= q.frontLength then
            ...
            PhysicistQueue(q.front.Value, (q.rBackLength + q.frontLength), (lazy (q.rBack |> List.rev |> List.append q.front.Value)), 0, [])
    ...
    //to do: create 2nd function (in background thread ?) in reflection by switching order of 'State & 'T parameters
    //       and do a foldback instead of rev on rBack list -- similar for foldBack
        let s = List.fold f state q.prefix
    ...
    member this.Rev() = PhysicistQueue<'a>((List.rev rBack), rBackLength, (lazy (List.rev rBack)), frontLength, (List.rev  front.Value)) |> PhysicistQueue.check
    ...
    member this.Uncons() = 
        match prefix with
        | hd::_ -> hd, (PhysicistQueue(prefix, (frontLength - 1), (lazy front.Value.Tail), rBackLength, rBack) |> PhysicistQueue.check)
        | _ -> raise Exceptions.Empty

    ///returns option first element and tail
    member this.TryUncons() =  
       match prefix with
        | hd::_ -> Some(hd, (PhysicistQueue(prefix, (frontLength - 1), (lazy front.Value.Tail), rBackLength, rBack) |> PhysicistQueue.check))
        | _ -> None
