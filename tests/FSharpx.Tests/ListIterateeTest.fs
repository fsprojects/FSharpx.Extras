...
let ``test takeWhile should take anything before the first space``() =
  ...
  actual |> shouldEqual (List.ofSeq "Hello")
...
let ``test takeUntilComma should take until the first comma``() =
  ...
  actual |> shouldEqual (List.ofSeq "blah")
