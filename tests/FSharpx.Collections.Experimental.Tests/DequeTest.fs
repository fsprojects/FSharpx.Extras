module FSharpx.Collections.Experimental.Tests.Deque

open System
open FSharpx.Collections.Experimental
open FSharpx.Collections.Experimental.Deque
...
let ``rev empty dqueue should be empty``() =
    empty() |> rev |> isEmpty |> should equal true
...
[<Test>]
let ``tryRemoveempty``() =
    empty() |> tryRemove 0 |> should equal None
...
[<Test>]
let ``deprecated structure still works``() =
    let q = empty()
