module FSharpx.Collections.Experimental.Tests.SkewBinaryRandomAccessListTest

open System
open FSharpx.Collections.Experimental
open FSharpx.Collections.Experimental.SkewBinaryRandomAccessList
...
[<Test>]
let ``lookup length 2``() =
    (((empty() |> cons "a" |> cons "b" |> lookup 0) = "b") && ((empty() |> cons "a" |> cons "b" |> lookup 1) = "a")) |> should equal true
...
let ``lookup length 3``() =
    ...
    (((len3 |> lookup 0) = "c") 
    && ((len3 |> lookup 1) = "b") 
    && ((len3 |> lookup 2) = "a")) |> should equal true
...
let ``tryLookup length 1``() =
    let a = empty() |> cons "a" |> tryLookup 0 
...
[<Test>]
let ``update length 1``() =
    empty() |> cons "a" |> update 0 "aa"|> lookup 0 |> should equal "aa"
